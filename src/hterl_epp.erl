%% PROMINENT NOTICE:
%% This file is a modified version of the file epp.erl in the
...
-export([scan_hterl_form/1]).
-export([parse_erl_form/1,macro_defs/1]).
...
scan_hterl_toks(St) ->
    case io:scan_erl_form(St#epp.file, '', St#epp.location, [return_white_spaces]) of
        {ok,Toks,Cl} ->
            case lex(Toks) of
                [] -> {eof,Cl};
                Toks1 -> {ok,Toks1,Cl}
